...
const StatsPanel: React.FC<StatsPanelProps> = ({ stats, currentWeight }) => {
    ...
    if (currentWeight > encumbered) {
        barColor = 'bg-dnd-gold';
        statusText = 'Нагружен';
        statusColor = 'text-dnd-gold';
    }
    if (currentWeight > maxCapacity) {
        barColor = 'bg-dnd-danger';
        statusText = 'Перегруз';
        statusColor = 'text-dnd-danger';
    }
    ...
    return (
        <div className="bg-dnd-panel border-t border-dnd-muted/50 p-3 flex items-center gap-4 shadow-lg z-30 sticky bottom-0">
            <div className="flex items-center gap-3 min-w-max">
                <div className={`p-2 rounded-full transition-colors ${currentWeight > maxCapacity ? 'bg-dnd-danger text-white' : 'bg-dnd-dark text-dnd-muted'}`}>
                    <Weight size={18} />
                </div>
                <div className="flex flex-col">
                    <span className="text-[10px] uppercase text-dnd-muted leading-none mb-0.5">Общий вес</span>
                    <div className="flex items-baseline gap-1">
                        <span className={`text-lg font-bold leading-none ${currentWeight > maxCapacity ? 'text-dnd-danger' : 'text-dnd-text'}`}>
            ...
            <div className="flex-1 flex flex-col justify-center gap-1">
                <div className="w-full bg-dnd-dark h-2.5 rounded-full overflow-hidden relative border border-dnd-muted/20">
                    ...
                    <div
                        ...
                        style={{ left: `${(encumbered / maxCapacity) * 100}%` }}
                        title={`Легкая нагрузка: ${encumbered}`}
                    />
                </div>
                <div className="flex justify-between text-[10px] text-dnd-muted px-0.5 select-none">
                    <span>0</span>
                    <span className="hidden sm:inline">Нагрузка ({encumbered})</span>
                    <span>Макс ({maxCapacity})</span>
                </div>
            </div>

            <div className={`hidden sm:flex flex-col items-end min-w-[80px] text-right`}>
                <span className="text-[10px] uppercase text-dnd-muted leading-none mb-0.5">Состояние</span>
