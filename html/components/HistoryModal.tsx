...
const HistoryModal: React.FC<HistoryModalProps> = ({ isOpen, onClose, history, onClear }) => {
  ...
  const handleSecretClick = () => {
      ...
      if (next >= 10) { onClear(); setSecretClickCount(0); } else { setSecretClickCount(next); }
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
      <div className="bg-dnd-panel w-full max-w-lg rounded-xl border border-dnd-gold shadow-2xl flex flex-col max-h-[80vh]">
        <div className="flex items-center justify-between p-4 border-b border-dnd-muted">
          <div className="flex items-center gap-2 text-dnd-gold"><History size={24} /><h2 className="text-xl font-bold font-serif">История Изменений</h2></div>
          <button onClick={onClose} className="text-dnd-muted hover:text-white transition"><X size={24} /></button>
        </div>
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
            {history.length === 0 ? (<div className="text-center text-dnd-muted py-10 opacity-50"><History size={48} className="mx-auto mb-2" /><p>История пуста</p></div>) : (history.map((entry, index) => {
                    const showDate = index === 0 || new Date(entry.timestamp).getDate() !== new Date(history[index - 1].timestamp).getDate();
                    return (
                        <div key={entry.id}>
                            {showDate && (<div className="text-xs text-dnd-muted text-center my-2 border-b border-dnd-muted/20 pb-1 font-bold">{formatDate(entry.timestamp)}</div>)}
                            <div className="flex gap-3 items-start p-2 rounded hover:bg-white/5 transition">
                                <div className="mt-1 opacity-80">{getIcon(entry.type)}</div>
                                <div className="flex-1"><p className="text-sm text-dnd-text leading-tight">{entry.description}</p><span className="text-[10px] text-dnd-muted">{formatTime(entry.timestamp)}</span></div>
                            </div>
                        </div>
                    );
                }))}
        ...
        <div className="p-4 border-t border-dnd-muted bg-dnd-dark/30 rounded-b-xl flex justify-center items-center">
            <span onClick={handleSecretClick} className="text-xs text-dnd-muted select-none cursor-default">Всего записей: {history.length}</span>
        </div>
      </div>
    </div>
  );
